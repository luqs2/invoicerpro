<template>
  <div class="page">
                  ...
                  <template v-if="showDate || showVehicleNo">
                    <div v-if="showDate" class="li-field">
                      ...
                      <input
                        ...
                        type="date"
                        :value="item.date"
                        @input="debouncedUpdateItem(idx, { date: ($event.target as HTMLInputElement).value })"
                      >
                    </div>
                    <div v-if="showVehicleNo" class="li-field">
                      ...
                      <input
                        ...
                        placeholder="—"
                        @input="debouncedUpdateItem(idx, { vehicle_no: ($event.target as HTMLInputElement).value })"
                      >
                    </div>
                  </template>
                  <div class="li-field">
                  ...
                  <template v-if="showUom">
                    <div class="li-field">
                      ...
                      <input
                        ...
                        :value="item.uom"
                        placeholder="MT"
                        @input="debouncedUpdateItem(idx, { uom: ($event.target as HTMLInputElement).value })"
                      >
                    </div>
                  </template>
                  <div class="li-field">
    ...
    <MobileActionBar
      :formatted-total="formatCurrency(Number(form.amount), form.currency)"
      :saving="saving"
      :export-disabled="saving"
      :send-disabled="!form.client_id"
      @export="exportPdf"
      @send="openSendDialog"
      @save="save"
    />
...
.card-subtitle { font-size: 12px; color: #414846; margin-left: auto; }
...
.inv-status-draft    { background: #f4ede3; color: #414846; }
...
.inv-status-cancelled{ background: #f4ede3; color: #414846; }
.inv-status-—        { background: #f4ede3; color: #414846; }
