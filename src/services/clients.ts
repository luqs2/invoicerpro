...
export const clientService = {
  async getAll(userId: string) {
    return supabase
      .from('clients')
      .select('id, user_id, name, email, phone, company, address, notes, attn, created_at')
      .eq('user_id', userId)
      .order('name')
  },

  async getById(id: string) {
    return supabase.from('clients').select('id, user_id, name, email, phone, company, address, notes, attn, created_at').eq('id', id).single()
  },
